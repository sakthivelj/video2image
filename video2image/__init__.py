...
Examples:
  ...
  video2image --save-config --jpeg-quality 90 --frame-step 3
    ...
    frame_group.add_argument("--start-time", type=float, 
                             help="Start time in seconds (e.g., 10.5)")
    frame_group.add_argument("--end-time", type=float, 
                             help="End time in seconds")
    frame_group.add_argument("--frame-range", type=parse_frame_range, 
                             help="Frame range (e.g., '100-200' or '100')")
    
    # Naming Customization
    naming_group = parser.add_argument_group("Naming Customization")
    naming_group.add_argument("--naming-pattern", 
                              help="Custom naming pattern (e.g., '{video}_{frame:04d}_{time}')")
    naming_group.add_argument("--prefix", default="", 
                              help="Prefix for output filenames")
    naming_group.add_argument("--suffix", default="", 
                              help="Suffix for output filenames")
    naming_group.add_argument("--include-timestamp", action="store_true", 
                              help="Include timestamp in filename")
    
    # Image Quality
    quality_group = parser.add_argument_group("Image Quality")
    quality_group.add_argument("-f", "--format", default="jpg", choices=["jpg", "png"], 
                               help="Image format (default: jpg)")
    quality_group.add_argument("--jpeg-quality", type=int, default=95, 
                               help="JPEG quality 1-100 (default: 95)")
    quality_group.add_argument("--png-compression", type=int, default=6, 
                               help="PNG compression 0-9 (default: 6)")
    quality_group.add_argument("--resize-width", type=int, 
                               help="Resize width (maintains aspect ratio if height not specified)")
    quality_group.add_argument("--resize-height", type=int, 
                               help="Resize height (maintains aspect ratio if width not specified)")
    quality_group.add_argument("--grayscale", action="store_true", 
                               help="Convert frames to grayscale")
    
    # Smart Features
    smart_group = parser.add_argument_group("Smart Features")
    smart_group.add_argument("--scene-detection", action="store_true", 
                             help="Enable scene change detection")
    smart_group.add_argument("--scene-threshold", type=float, default=30, 
                             help="Scene change threshold 0-255 (default: 30)")
    smart_group.add_argument("--motion-based", action="store_true", 
                             help="Enable motion-based extraction")
    smart_group.add_argument("--motion-threshold", type=float, default=5, 
                             help="Motion threshold (default: 5)")
    smart_group.add_argument("--remove-duplicates", action="store_true", 
                             help="Remove duplicate frames")
    
    # Batch Processing
    batch_group = parser.add_argument_group("Batch Processing")
    batch_group.add_argument("--recursive", action="store_true", 
                             help="Search for videos recursively in directory")
    batch_group.add_argument("--patterns", nargs='+', 
                             help="Custom file patterns (e.g., '*.mp4 *.mkv')")
    batch_group.add_argument("--parallel", action="store_true", 
                             help="Process multiple videos in parallel")
    batch_group.add_argument("--max-workers", type=int, default=4, 
    ...
    # Help
    parser.add_argument("-H", "--show-help", action="help", 
    ...
    if args.save_config:
        if not args.input:
            print("Error: --input is required when saving config with video-specific settings")
            return
        config = {
    ...
    # Handle directory with multiple video files
        ...
        if args.recursive:
            ...
            patterns = args.patterns or ["*.mp4", "*.mkv", "*.avi", "*.mov", 
                                        "*.MP4", "*.MKV", "*.AVI", "*.MOV"]
            video_files = []
            for ext in patterns:
